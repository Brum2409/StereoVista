...
| 7 Tools & export | ✅ done (MSVC Release x64 builds + links clean) | `StereoVista-vulkan` | **`core::ToolManager`** (`Core/ToolManager.h` + `src/Core/ToolManager.cpp`): the one registry of interactive MODES — at most one active, activating one exits the others, `Esc` exits the active one. It stores **closures, not tool objects**, because the tools are heterogeneous (measurement lives in a Plugin that owns its own enabled flag; the clip-plane tool is app-owned), and `active()` **asks** each tool rather than caching a pointer — so a plugin the user toggles from its own window still reads as the active tool everywhere. Registration is `src/App/ToolOps.cpp`. **Every tool surface now renders from the registry:** the Tools menu (registry commands), the `Ctrl+K` palette, the **status-bar chip** (click to exit), the **Inspector Tool card**, and — via `Services::tools()` — the Pass-8 viewport toolbar. **`Esc` was de-hard-coded**: `escapeAction()` used to name the clip tool explicitly; it now calls `toolManager_.deactivateAll()`, so *every* tool (including ones added later) exits on Esc with no edit. **Plugins gained one additive hook**, `onRenderInspector(ctx)` (`PluginManager::renderInspector` calls it for enabled plugins only) — a tool draws its options in the Inspector's Tool card instead of a floating window. **Any plugin declaring `PluginCategory::Tool` is auto-registered as a mode** (menu + palette + rebindable command + status chip + Esc + exclusivity) with *zero* registration code — the §16.1 recipe paying off. `MeasurementPlugin` now renders its options (mode/labels/units) in the card; its measurement *list* is deliberately gone from the plugin (measurements have been Outliner rows since Pass 1). **ClipPlanes panel DISSOLVED** (§5.1): its content became `drawClipPlaneToolOptions()` in the Tool card, its planes are Outliner rows, its window/menu item/command are removed — same controls, better home (C10). **`Gui::Exporters`** (`Gui/Exporters.h` + `src/Gui/Exporters.cpp`): the export registry mirroring the import pipeline — exporters declare `applies(selection)` + `run(selection)`, and `File ▸ Export…` (new `file.export` command) renders from it, selection-aware. Built-ins: screenshot, scene (.scene), point cloud. A new exporter (mesh format, stereo screenshot mode, web package) is ONE registration. `docs/PLUGINS.md` updated with the hook + the tools-are-modes contract. **Deviations:** the exporter registry lives in the **Gui** layer rather than Core (unlike `ImportService`, whose *planner* is genuinely pure) because every executor already exists behind `Gui::Services` — an exporter is a title, a predicate and a call, with no Vulkan/loader dependency to hide; if one ever needs the device it grows a Services method exactly as `importFiles` did. The Outliner's context-menu "Export" entry is not wired yet (File ▸ Export… + the Inspector's Export section cover it); point-cloud format options still live in the Inspector's Export section (the File ▸ Export… entry uses the binary-PLY interchange default). |
| 8 Viewport & alive | ✅ done (MSVC Release x64 verified by owner) | `StereoVista-vulkan` | **Per-viewport toolbar** (`GuiSystem::drawViewportToolbar`): gizmo segmented control · view ▾ (7 standard views) · Frame · shading · the ToolManager's tool chips · hide-UI. It acts on **the viewport it is drawn in** (§5.2), never viewport 0 — which needed new viewport-scoped Services (`activeViewport()`, `viewportCamera(i)`, `applyStandardView(i, view)`, `frameItemsIn(i, refs)`), because `Services::camera()` returns the **primary** camera and a toolbar built on it would have silently driven viewport 0 from every viewport. **The input trap:** the toolbar is a **child window**, so ImGui routes its clicks away from the scene image — but `ViewportPanelState::hovered` was already latched from `IsItemHovered()` on the image, so the toolbar returns "I own the pointer" and the panel **clears `hovered` before reporting it**. Without that, clicking a toolbar button would also start an orbit in the scene behind it. **Standard views** are registered commands (`view.std.*`), so the toolbar, the palette and the F1 sheet drive the same code (C5). **Motion fix:** `frameItems` used to `SetState()` — it now flies via `StartStateAnimation` (§15), snapping only under reduce-motion; the top/bottom degenerate up-vector is handled. **Status bar completed:** stereo/VR chip added (tool chip + autosave whisper landed in Passes 7/5). **SceneStats** (`Services::sceneStats()`): models/meshes/lights/groups/layers/clouds, triangles, points, cloud VRAM and the true device-local VRAM budget (`rhi::Device::MemoryBudget`) — feeds the status bar and the hint triggers. **F1 shortcut overlay** (`drawShortcutOverlay`): hold `F1` for a live keymap generated from the CommandRegistry + ShortcutMap (so a rebind or a brand-new command appears for free), `Shift+F1` pins it; drawn **outside** the master GUI toggle, because someone who hid the UI with `G` still needs the keymap. `Services::shortcutHeld()` keeps GLFW out of the Gui layer. **HintEngine** (`Gui/HintEngine.h/.cpp`): one hint at a time, 45 s cooldown, dismiss-forever (persisted in `Settings::Ui::Hints::dismissed`), master pref. Launch set: frame-selected after the first object, "measurements live in the Outliner", grouping tip at 5+ loose objects, and a performance guardian that fires only after **sustained** low FPS with an expensive setting on — and **offers** `[Lower it] [Keep]`, never changing a setting by itself. |
| 9 Harmony | ✅ done | `StereoVista-vulkan` | Harmony audit + dead-code sweep. **Docs trued up** (this board, CLAUDE.md migration state). **§16.1 recipe re-verified against the shipped code:** a tool that follows the recipe touches ZERO existing UI files — it self-registers (`REGISTER_PLUGIN` / `ToolManager` auto-registration for `PluginCategory::Tool`), gets its command + menu + palette + status chip + Esc + exclusivity for free, draws options via the additive `onRenderInspector` hook, and if it produces objects it registers an Outliner provider (which the palette's object search consumes automatically) + an Inspector editor (last-wins registry) + `SceneDocument` v3 fields; a file-moving tool registers an importer/exporter. Every seam is append-only (C6), confirmed by walking `MeasurementPlugin` (Tool plugin), the clip tool (app-owned tool) and the built-in exporters — none required edits to menu/palette/inspector/outliner source. **Coherence spot-checks:** all UI built from UiKit (C1); every action runs through `CommandRegistry::run` (C5); no compiled TU includes a Vulkan/renderer header from the Gui layer (Services facade intact); no compiled TU includes a GL-era reference header. **C4996 sweep clean** (deprecated CRT only appears in `#else` non-MSVC arms of the guarded time helpers). **Bindless texture-slot leak fixed** (the P2 deviation): `applyMaterialTexture` now retires the previous slot texture(s) via `MaterialSystem::freeTexture(old, frameRetireValue())`, deduping a shared index across meshes so it is never double-parked. **Dead-code deletion:** the fully-ported **`GUI.cpp`/`Gui.h`** (→ `panels/*` + UiKit), **`ShortcutManager.*`** (→ `core::ShortcutMap`) and **`SnapshotManager.*`** (→ Pass-3 Snapshots/`SceneDocument`) were removed from the tree and the `.vcxproj`(+`.filters`) — they were `ExcludedFromBuild` and referenced by nothing (git history keeps them). **Kept as active references** (still genuinely unported, per CLAUDE.md): `SceneManager.cpp` (merge/hierarchy edge cases), `CursorSynchronizer`, `CursorPresets`, `SpaceMouseInput`/`ThreeDConnexionSync`, `CursorPreview3D`, and `GuiTypes.h` (still `#include`d by the excluded `SpaceMouseInput.h`). **Deviations:** the §15 QoL catalog is treated as audited-not-exhaustive — the high-value items landed across P0–P8 (tooltips w/ live shortcuts, truncated-text tooltips, unsaved dot, toast `[Undo]`, per-section resets + modified dots, motion via `Anim01`/`reduceMotion`); a universal per-field right-click Reset/Copy/Paste was scoped OUT (it needs per-`T` clipboard serialization the templated `EditRow` can't do generically, and per-row defaults `EditRow` doesn't carry — `SectionReset` covers reset at section grain) rather than shipped half-done. |
...
- 2026-07-12 — Pass 2 landed (Inspector). The **editor registry**
  ...
  cleanup.
- 2026-07-12 — Passes 3–8 landed (History/Snapshots, palette, import/hub/autosave,
  Settings overhaul, tools/export, viewport toolbar & status/hints). See each
  Status Board row for scope + deviations.
- 2026-07-12 — Pass 9 (Harmony) landed. Docs trued up; §16.1 recipe re-verified
  (a recipe-following tool edits zero existing UI files); C1/C5/Services/GL-header
  invariants spot-checked clean; the P2 bindless texture-slot leak fixed
  (`applyMaterialTexture` retires prior slot textures, dedup-guarded). **Dead-code
  deletion:** the fully-ported `GUI.cpp`/`Gui.h`, `ShortcutManager.*` and
  `SnapshotManager.*` removed from the tree + project files (git keeps history).
  `SceneManager`, cursor sync/presets, SpaceMouse/3DxSync, `CursorPreview3D` and
  `GuiTypes.h` stay as active references for their still-unported systems.
