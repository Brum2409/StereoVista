...
| **lepcc** | github.com/Esri/lepcc | master @ **63a30e2** (2022-08-12) ✅ vendored | `src/*.{cpp,h}` minus `Test_C_Api.cpp`, + `include/{lepcc_c_api.h, lepcc_types.h}` (`lepcc_tpl_api.h` is unreferenced — skipped), LICENSE.TXT + NOTICE.TXT, under `headers/libs/lepcc/` | Apache-2.0. Compiled static — no defines needed (`LEPCC_EXPORT` empties without `_WINDOWS`/`LEPCC_EXPORTS`). Decoder entry points used: `lepcc_getPointCount`/`lepcc_decodeXYZ`, `lepcc_getRGBCount`/`lepcc_decodeRGB`, `lepcc_getIntensityCount`/`lepcc_decodeIntensity` — ONE context per call (contexts carry per-decode state; workers stay independent). BONUS: the repo ships a real ArcGIS-cooked PCSL sample (`testData/SMALL_AUTZEN_LAS_All.slpk`) — see `StereoVista/testdata/README.md` |
...
- [x] Vendor lepcc (§3). PCSL branch in `I3SLayer` (leaf-only node model,
      OBB-only — parses since M0) + lepcc-xyz/rgb/intensity decode.
      *(Decode lives in a NEW `I3SPointCloud.{h,cpp}` module rather than
      inside the mesh-focused I3SGeometry — same NodeFrame, but LEPCC blobs
      hold ABSOLUTE layer-SR coordinates (node-relative subtraction happens
      in the decoder, in double). Resource naming validated against the real
      lepcc-repo PCSL sample: 1.x tags LEPCC blobs `.bin.pccxyz`/`.pccint`/
      `.pccrgb` (raw, NOT gzipped — lepcc is its own compression), raw
      columns stay `.bin.gz`; both probed. `AttributeField` gained the PCSL
