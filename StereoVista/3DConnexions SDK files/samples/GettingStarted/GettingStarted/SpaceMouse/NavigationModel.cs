...
namespace TDx.GettingStarted.Navigation
{
    ...
    internal partial class NavigationModel : INavigation3D
    {
        ...
        // For perspective the navigation library uses a conical beam for hit-testing.
        private readonly ConeBeam3 hitBeam = new ConeBeam3();

        private bool enable = false;
        private string profile = default(string);
        private Matrix4 selectionTransform = default(Matrix4);
        ...
        /// <summary>
        /// Gets the navlib property interface.
        /// </summary>
        public INavlibProperty Properties => this.navigation3D.Properties;
        ...
        {
            throw new TDx.SpaceMouse.Navigation3D.NoDataException("This camera does not have a movable target object");
        }
        ...
        {
            throw new System.InvalidOperationException("This camera does not have a movable target object");
        }
        ...
        Point IView.GetPointerPosition()
        {
            ...
            System.Drawing.Point screenPosition = System.Windows.Forms.Control.MousePosition;

            // Convert to client coordinates
            System.Drawing.Point clientPosition = this.viewportVM.Viewport.PointToClient(screenPosition);
            ...
            Vector4 screenPos = new Vector4(
                ((float)clientPosition.X / viewport[2] * 2.0f) - 1.0f,
                1.0f - (((float)clientPosition.Y / viewport[3]) * 2.0f),
                0.0f,
                1.0f);

            // Unproject the screen coordinates to world coordinates
            Vector4 worldPos = Vector4.Transform(screenPos, Matrix4.Invert(projectionMatrix) * Matrix4.Invert(modelViewMatrix));
            if (worldPos.W > float.Epsilon)
            {
            ...
            return new Point(worldPos.X, worldPos.Y, worldPos.Z);
        }
        ...
        {
            Point3 hit;
            if (this.hitBeam.Intersects(this.viewportVM.ModelBounds, out hit))
            {
                return hit.AsPoint();
            }
        ...
        {
            this.hitBeam.Origin = value.AsPoint3();
        }
        ...
        {
            this.hitBeam.Direction = value.AsVector3();
        }
        ...
        void IHit.SetLookAperture(double value)
        {
            ...
            this.hitBeam.Radius = (float)value / 2 / this.viewportVM.Camera.NearPlaneDistance;
        }
        ...
        {
            this.viewportVM.PivotPosition = value.AsPoint3();
        }
        ...
        {
            return this.viewportVM.PivotReadOnly;
        }
        ...
        private void MotionChangedHandler(object sender, MotionEventArgs eventArgs)
        {
            this.dispatcher.InvokeAsync(() =>
            {
                ...
                if (!eventArgs.IsNavigating)
                {
                    ...
                    this.selectionTransform = Matrix4.Identity;
                }
            });
        }
