...
int main() {
  ...
  *spaceMouseCameraPtr = spaceMouseCamera;
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    GLuint xrLeftTexture = 0; // saved for the desktop mirror blit
    if (g_xrSession && preferences.openxrSettings.enabled) {
      ...
      if (g_xrSession && g_xrSession->isRunning()) {
        ...
        if (posesValid) {
          ...
          // This maps the camera orbit/pan controls to the scene origin while
          // head-tracking and per-eye IPD are provided by the XR runtime.
          ...
          // Left eye
          xrLeftTexture = g_xrSession->acquireSwapchainImage(0);
          if (xrLeftTexture) {
    ...
    // Desktop mirror: blit the left-eye XR texture to the desktop viewport.
    // Only done when XR rendered successfully AND the user wants the mirror.
    // Skipped entirely when XR is disabled (zero cost).
    if (xrFrameRendered && preferences.openxrSettings.mirrorToWindow && xrLeftTexture) {
      // Re-attach the left swapchain texture so we can read from it.
      GLuint mirrorFBO = g_xrSession ? g_xrSession->fbo() : 0;
      if (mirrorFBO) {
        ...
        if (isStereoWindow) {
          // For quad-buffer stereo windows blit to both eyes on the desktop.
      ...
      // Render the desktop GUI on top of the mirror image.
      if (showGui) {
    ...
    // If XR is running but mirror is disabled: skip desktop render entirely.
