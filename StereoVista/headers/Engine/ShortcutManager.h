...
struct KeyBinding {
    ...
    static KeyBinding MouseButton(int button, bool ctrl = false, bool alt = false, bool shift = false) {
        KeyBinding binding;
    ...
    bool isValid() const {
        return keyCode != -1 || mouseButton != -1;
    }
...
public:
    std::string name;
    std::map<ShortcutAction, std::vector<KeyBinding>> bindings; // Up to 2 bindings per action
    ...
    void setBinding(ShortcutAction action, const KeyBinding& binding, int slot = 0);

    // Remove a binding
    void clearBinding(ShortcutAction action, int slot);

    // Check if a binding is already used by another action
    std::optional<ShortcutAction> findConflict(const KeyBinding& binding, ShortcutAction excludeAction) const;
...
public:
    ...
    const std::vector<std::string>& getProfileNames() const { return profileNames; }
    ...
    bool exportProfileToFile(const std::string& profileName, const std::string& filepath) const;
